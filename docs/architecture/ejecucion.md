...
Los reintentos automáticos ocultos del SDK de inferencia se desactivan y el cliente fija un solo intento de transporte. El coordinador permite hasta dos reintentos adicionales sólo por throttling inequívoco, con ordinal y uso propios. Espera 60 segundos antes de cada reintento para dejar recuperar la capacidad de Bedrock; durante la espera comprueba cancelación cada 5 segundos y no inicia otra llamada si falta tiempo para la llamada y la reserva de guardado. Si persiste el rechazo, cierra con causa `throttled`. Esta política sigue separada de cualquier retry de escritura. Una respuesta semánticamente inválida termina con error; no se corrige ni se vuelve a preguntar con contexto adicional. Un timeout o corte de transporte cuyo procesamiento sea incierto termina con uso desconocido para esa llamada. Reintentar una escritura de los mismos bytes o consultar una transacción ambigua no vuelve a llamar al modelo.
