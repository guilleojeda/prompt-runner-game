...
**Estado del proyecto:** acceso Cognito, configuración persistida y ejecución real del nivel principal con recompensa mediante AgentCore Runtime, Strands y Bedrock. Probar guarda la configuración visible y fija el intento; la animación opcional presenta las acciones guardadas antes del resultado. El historial propio permite volver a ver los intentos del contrato vigente sin inferencia. La llave, la comparación y el segundo recorrido corresponden a fases posteriores. El nombre del juego es provisional.
...
El límite de configuración es 65.536 bytes de JSON UTF-8, incluidos los contratos fijos de las habilidades. El editor lo informa y no trunca textos. Las copias pendientes del editor permanecen sólo en memoria: un cierre abrupto puede perder cambios que el servidor no confirmó. Como excepción transitoria, después de **Probar** se guarda en `sessionStorage` la clave de admisión, su versión, el snapshot exacto y la elección de Animación para recuperar una respuesta perdida; la referencia foreground conserva el intento cuyo cálculo o resultado aún se debe recuperar y pasa al nuevo intento al admitirlo. El servidor determina la recuperación: si el intento es terminal y `presentationComplete=false`, al volver se reproduce desde el principio aunque esta pestaña ya hubiera mostrado la animación. Si la confirmación del ACK falla, el resultado queda disponible y se puede reintentar; tras una recarga se vuelve a consultar el estado del servidor. La referencia foreground se elimina cuando el servidor confirma el ACK, al cerrar sesión o al cambiar de cuenta.
...
El único nivel vigente, `principal-recompensas-v3` (versión 3), tiene siete tramos: suelo, pozo, suelo, rama, barrera, plataforma y suelo. La salida está en el apoyo 7 y el límite es de 16 acciones. La barrera está baja en turnos pares y alta en impares; la plataforma es suelo cuando el turno es múltiplo de tres y pozo en los demás. Una recompensa de 25 puntos está en el apoyo 2: Agarrar objeto la recoge únicamente desde ese apoyo, consume un turno y la conserva en el inventario. Pasar por allí no la recoge. Avanzar y Retroceder caminan; Saltar y Agacharse y avanzar reciben dirección; Nadar consume un turno sin mover al robot. Esperar deja pasar un turno sin moverse y permite que cambie el terreno periódico. Las descripciones explican las habilidades al agente, pero no cambian la física.
...
Desde Probar hasta el resultado se bloquean edición, nuevos intentos, historial y cierre de sesión. Durante el cálculo podés cancelar; durante la reproducción no hay controles de pausa, avance, retroceso o salto. Si una llamada ya estaba en vuelo, su consumo se conserva aunque no se publique otra acción. Cerrar el navegador no cancela el trabajo del servidor. Al volver, recuperás el cálculo o la presentación según el resumen del servidor, sin crear otro intento. Un intento terminal con `presentationComplete=false` reproduce otra vez desde el principio; uno con `presentationComplete=true` abre el resultado directamente. Si el ACK falla después de mostrar el resultado, podés reintentarlo; si iniciás otro intento, éste pasa a ser la referencia de recuperación y el anterior sigue disponible en el historial.
