...
El límite de configuración es 65.536 bytes de JSON UTF-8, incluidos los contratos fijos de las habilidades. El editor lo informa y no trunca textos. Las copias pendientes del editor permanecen sólo en memoria: un cierre abrupto puede perder cambios que el servidor no confirmó. Como excepción transitoria, después de **Probar** se guarda en `sessionStorage` la clave de admisión, su versión, el snapshot exacto y la elección de Animación para recuperar una respuesta perdida; la referencia foreground conserva el intento cuyo cálculo o resultado aún se debe recuperar y pasa al nuevo intento al admitirlo. Una lista separada por cuenta conserva los IDs de las presentaciones que ya llegaron al último frame o se omitieron con **Ver resultado**, mientras falte confirmar el ACK del servidor. Al volver, se restaura el resultado si el ACK aún está pendiente y se reintenta sólo ese ACK idempotente, sin volver a reproducir lo que ya se vio. Las referencias no son una preferencia ni un borrador alternativo y se eliminan al confirmar, cerrar sesión o cambiar de cuenta.
