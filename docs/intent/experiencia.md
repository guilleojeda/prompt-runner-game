...
Por ahora no hay selector de nivel: se juega el recorrido estático. El toggle **Animación** está disponible y su preferencia se guarda en el servidor por usuario; cada intento conserva el valor elegido. Las referencias de recuperación de la pestaña no sustituyen el registro del servidor. Durante una admisión ya iniciada, la pestaña puede conservar transitoriamente la clave, versión, elección de Animación y snapshot exactos en `sessionStorage` para reintentar la misma solicitud tras una respuesta perdida. La referencia foreground conserva el intento cuyo cálculo o resultado todavía se debe recuperar; permanece en A mientras su ACK pendiente no se confirme, hasta que una nueva admisión B tome el primer plano. Por separado, una lista por cuenta conserva los IDs cuya animación ya llegó al último frame o cuyo resultado se eligió explícitamente, pero cuyo ACK de `presentationComplete` aún no fue confirmado. Al volver, la pestaña reintenta ese ACK idempotente sin volver a reproducir lo ya visto; si no quedó esa constancia local, reproduce desde el principio. Estas referencias no guardan un borrador alternativo ni funcionan como preferencia.
...
El estado del intento en curso y el valor de **Animación** fijado al hacer clic en **Probar** se conservan en el servidor. Si la página se recarga durante el cálculo, recupera el intento y vuelve al estado de cálculo con los controles bloqueados; no crea otra inferencia ni otra cuota. Si el job ya terminó pero la interfaz todavía debe reproducir automáticamente, recupera el registro cerrado y mantiene bloqueadas la edición y la consulta del resultado hasta terminar la reproducción. La reproducción recuperada no llama al modelo. Si esta pestaña ya llegó al último frame o la persona eligió **Ver resultado**, pero no recibió confirmación del ACK, conserva el ID por separado y al volver reintenta sólo el ACK y restaura el resultado de A desde el foreground; no vuelve a mostrar una presentación que ya se completó visualmente. Si el ACK no había sido confirmado visualmente, se conserva la recuperación foreground y la reproducción vuelve a empezar. Si B se admite mientras el ACK de A sigue pendiente, el foreground pasa a B y el ACK de A continúa reintentándose por separado.
